  ...
  globalEvents: function() {
    ...
    document.querySelectorAll(".take-button").forEach(function(el, i) {
      el.addEventListener("mouseenter", function(e) {
        ...
        TweenMax.to(this.querySelector("i"), 0.5, {
          y: 5,
          height: "40px",
          skewX: -31,
          ease: Power2.easeOut
    ...
    document.querySelectorAll(".btn-skew").forEach(function(el, i) {
      el.addEventListener("mouseenter", function(e) {
        ...
        TweenMax.to(this.querySelector("span"), 0.5, {
          color: "#ee412a",
          ease: Power2.easeOut
        });
        TweenMax.to(this.querySelector("i"), 0.5, {
  ...
  if (e.type == "mouseenter") {
    ...
    tl.to(this.querySelector("a"), 0.6, { color: "#ee412a" }, "start")
      .to(
        ...
        0.6,
        { fill: "#ee412a", rotation: 180 },
        "start"
      )
      .to(
        document.querySelectorAll(
          ".header__nav_link:not(.active) > a, .header__right a"
        ),
        0.6,
        {
          color: function() {
            ...
            if (
              ...
              document.body.classList.contains("leisure")
            ) {
              color = "#2f2f2f";
            } else if (document.body.classList.contains("catalog")) {
              color = "#5ac0b0";
      ...
      .to(
        document.querySelectorAll(
          ".header__nav_link:not(.active) > a .icon, .header__right .icon"
        ),
        0.6,
        {
          fill: function() {
      ...
      .fromTo(
        this.querySelectorAll(".submenu .row"),
        0.8,
        { opacity: 0, y: -20 },
    ...
    let tl = new TimelineMax();
    tl.to(
      document.querySelectorAll(".header__nav_link > a, .header__right a"),
      0.6,
      {
        color: function() {
          ...
          if (
            ...
            return document.querySelector("header").classList.contains("fixed")
              ? "#2f2f2f"
              : "#ffffff";
          } else if (document.body.classList.contains("catalog")) {
            return "#5ac0b0";
      ...
      .to(
        document.querySelectorAll(
          ".header__nav_link > a .icon, .header__right .icon"
        ),
        0.6,
        {
          fill: function() {
...
const _homepage = function(page) {
  ...
  this.heroBgCover = function() {
    ...
    if (document.body.clientWidth / window.innerHeight > ratio) {
      ...
      TweenMax.set(video, {
        ...
        top: -(document.body.clientWidth / ratio - window.innerHeight) / 2,
        left: 0
      });
    } else {
      TweenMax.set(video, {
        ...
        top: 0,
        left: -(window.innerHeight * ratio - document.body.clientWidth) / 2
      });
    }
  };
  ...
  this.heroVideo = {
    ...
    controls: document.querySelector(
      ".homepage__hero .hero--player .player--controls"
    ),
    interval: null,
    active: false,
    loaded: false,
    ...
    time: function() {
      return {
        current:
          (Math.round(this.video.currentTime / 60) < 10
            ? "0" + Math.round(this.video.currentTime / 60)
            : Math.round(this.video.currentTime / 60)) +
          ":" +
          (Math.round(this.video.currentTime % 60) < 10
            ? "0" + Math.round(this.video.currentTime % 60)
            : Math.round(this.video.currentTime % 60)),
        duration:
          (Math.round(this.video.duration / 60) < 10
            ? "0" + Math.round(this.video.duration / 60)
            : Math.round(this.video.duration / 60)) +
          ":" +
          (Math.round(this.video.duration % 60) < 10
            ? "0" + Math.round(this.video.duration % 60)
            : Math.round(this.video.duration % 60))
    ...
    init: function() {
      let video = this;
      let src = document
        .querySelector(".hero--player video")
      ...
      // }
      // xhr.send();
      document.querySelector(".hero--player video").src =
        app.homepage.blobs.heroVideo || src;

      //this.controls.querySelector('[data-time="all"]').innerText = this.time().duration;
      this.video.addEventListener("click", function(e) {
        if (this.paused) {
          video.play();
          TweenMax.to(".cursor i.pause", 0.3, {
            scale: 1,
            ease: Power2.easeInOut
          });
          TweenMax.to(".cursor i.play", 0.3, {
            scale: 0,
            ease: Power2.easeInOut
          });
        } else {
      ...
      this.video.addEventListener("loadedmetadata", function(e) {
        video.controls.querySelector(
          '[data-time="all"]'
        ).innerText = video.time().duration;
      });
      this.video.addEventListener("timeupdate", function(e) {
        TweenMax.set(".player--controls .controls--bar i", {
          width: video.bar() + "%"
        });
      });
      document
        .querySelector('[data-action="homepage-hero-play"]')
        .addEventListener("click", function(e) {
          video.video.play();
          app.homepage.heroVideo.active = true;
          let tl = new TimelineMax();
          let h1 = [].slice
            .call(document.querySelectorAll(".homepage__hero h1 span"), 0)
            .reverse();

          tl.to(
            app.homepage.hero.displacementFilter.scale,
            0.2,
            {
              x: 0,
              y: 0,
              onComplete: function() {
                TweenMax.to(".homepage__hero h1 span", 0.2, {
                  opacity: 1,
                  onComplete: function() {
            ...
            .to(
              '[data-action="homepage-hero-play"]',
              0.3,
              { scale: 0, ease: Power2.easeIn },
              "cursor"
            )
            ...
            .staggerTo(
              h1,
              0.5,
              { rotationX: 90, opacity: 0, ease: Power2.easeIn },
              0.05,
              "-=0.3"
            )
            ...
            .fromTo(
              '[data-action="homepage-close-video"]',
              0.5,
              { scale: 1.4, opacity: 0 },
              { scale: 1, opacity: 1 }
            );
      ...
      document
        .querySelector('[data-action="homepage-close-video"]')
        .addEventListener("click", function(e) {
          ...
          tl.to(".cursor span", 0.6, { scale: 1 }, "cursor")
            ...
            .staggerFromTo(
              document.querySelectorAll(".homepage__hero h1 span"),
              0.8,
              { rotationX: 90, opacity: 0 },
              { rotationX: 0, opacity: 1, ease: Power2.easeOut },
              0.1,
              "-=0.3"
            )
            .to(app.homepage.hero.container, 0.3, { alpha: 1 })
            .to(".homepage__hero h1 span", 0.3, { opacity: 0 })
            .to(app.homepage.hero.displacementFilter.scale, 0.5, {
              x: 30,
              y: 30
            })
  ...
  this.loader = {
    progress: 0,
    resources: 1,
    scripts: ["pixi", "splitText", "swiper", "aos"],
    resourcesDone: 0,
    next: 0,
    step: 0,
    loaderReady: false,
    ...
    init: function() {
      ...
      TweenMax.to("#loader-logo .h", 20, {
        rotation: 360,
        transformOrigin: "88% 95%",
        ease: Power0.easeNone,
        repeat: -1
      });
      TweenMax.to("#loader-logo .m", 2, {
        rotation: 360,
      ...
      this.colorsAnim = new TimelineMax({ repeat: -1 })
        .to("#loader-logo .h, #loader-logo .m", 0.5, {
          fill: "#f8d4e4",
          ease: Power0.easeNone
        })
        .to("#loader-logo .h, #loader-logo .m", 0.5, {
      ...
      if (this.resources - this.resourcesDone == 1) {
        ...
        this.mySwiper.forEach(function(el, i) {
          if (el.el.closest(".homepage__services")) {
            TweenMax.set(
              el.el.querySelector(".swiper-slide-active .slide--photo"),
              { scale: 1.4 }
            );
          }
          el.on("slideChangeTransitionStart", function() {
            if (this.realIndex > this.previousIndex) {
              let tl = new TimelineMax();
              tl.to(
                //.to(this.el.querySelector('.swiper-slide-prev .slide--photo img'), 0.7, {opacity: 0}, 'start')
                this.el.querySelector(".swiper-slide-prev .slide--photo img"),
                1.4,
                { scale: 1.5 },
                "start"
              )
                .to(
                  this.el.querySelector(".swiper-slide-prev"),
                  0.4,
                  {
                    skewX: -30,
                    scale: 1.2,
                ...
                .fromTo(
                  this.el.querySelector(
                    ".swiper-slide-active .slide--photo img"
                  ),
                  1.4,
                  { scale: 1.5 },
                  { scale: 1 },
              ...
              let tl = new TimelineMax();
              tl.to(
                //.to(this.el.querySelector('.swiper-slide-next .slide--photo img'), 0.7, {opacity: 0}, 'start')
                this.el.querySelector(".swiper-slide-next .slide--photo img"),
                1.4,
                { scale: 1.5 },
                "start"
              )
                .to(
                  this.el.querySelector(".swiper-slide-next"),
                  0.4,
                  {
                    skewX: 30,
                    scale: 1.2,
        ...
        document.querySelector(
          ".homepage__hero .hero--bg"
        ).src = document
          .querySelector("[data-background]")
    ...
    loaded: function() {
      ...
      tl.to(
        "#loader-logo .h, #loader-logo .m",
        0.8,
        { fill: "#2f2f2f", ease: Power3.easeInOut },
        "arrows"
      )
        .to(
          "#loader-logo .h",
          0.8,
          { rotation: 360, transformOrigin: "88% 95%", ease: Power3.easeInOut },
          "arrows"
        )
        .to(
          "#loader-logo .m",
          0.8,
          { rotation: 360, transformOrigin: "96% 50%", ease: Power3.easeInOut },
          "arrows"
        )
        .to(
          "#loader-logo .m",
          0.8,
          { x: 0, y: 0, opacity: 0, ease: Power3.easeInOut },
          "morph"
        )
        .to(
          "#loader-logo .h",
        ...
        .to(
          ...
          {
            morphSVG:
              "M113.5,133.7l-14.3-47h15.7c0,0,10-1.3,19.6,10.4s45.7,56.1,45.7,56.1l-120-2.6l110.9-1.3l-43-52.2c0,0-5.7-6.5-13-6.5s-11.7,0-11.7,0L113.5,133.7z",
        ...
        .fromTo(
          ...
          0.8,
          { opacity: 0, scale: 1.4 },
          { opacity: 1, scale: 1, ease: Power3.easeInOut },
          "-=0.6"
        )
        ...
        .to([".loader .blind-left", ".loader .blind-right"], 0.8, {
          ...
          onComplete: function() {
            ...
            app.homepage.hero.hero.renderer.resize(
              app.homepage.hero.el.clientWidth,
              app.homepage.hero.el.clientHeight
            );
            app.homepage.hero.smart.x =
              (app.homepage.hero.el.clientWidth / 100) * 42;
            app.homepage.hero.smart.y =
              140 - (Math.floor(fs) / 100) * 7.563724377883669;
            app.homepage.hero.choise.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 6;
            app.homepage.hero.choise.y =
              140 - (fs / 100) * 7.563724377883669 + Math.floor(fs);
            app.homepage.hero.save.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 12.1;
            app.homepage.hero.save.y =
              140 -
              (Math.floor(fs) / 100) * 7.563724377883669 +
              Math.floor(fs) * 2;
            app.homepage.hero.time.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 18.2;
            app.homepage.hero.time.y =
              140 -
              (Math.floor(fs) / 100) * 7.563724377883669 +
              Math.floor(fs) * 3;

            app.homepage.hero.smart2.x =
              (app.homepage.hero.el.clientWidth / 100) * 42;
            app.homepage.hero.smart2.y =
              140 - (Math.floor(fs) / 100) * 7.563724377883669;
            app.homepage.hero.choise2.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 6;
            app.homepage.hero.choise2.y =
              140 - (fs / 100) * 7.563724377883669 + Math.floor(fs);
            app.homepage.hero.save2.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 12.1;
            app.homepage.hero.save2.y =
              140 -
              (Math.floor(fs) / 100) * 7.563724377883669 +
              Math.floor(fs) * 2;
            app.homepage.hero.time2.x =
              (app.homepage.hero.el.clientWidth / 100) * 42 +
              (app.homepage.hero.el.clientWidth / 100) * 18.2;
            app.homepage.hero.time2.y =
              140 -
              (Math.floor(fs) / 100) * 7.563724377883669 +
              Math.floor(fs) * 3;
            ...
            TweenMax.to(".homepage__description_bg .h", 20, {
              rotation: 360,
              transformOrigin: "88% 95%",
              ease: Power0.easeNone,
              repeat: -1
            });
            TweenMax.to(".homepage__description_bg .m", 2, {
              rotation: 360,
              transformOrigin: "96% 50%",
              ease: Power0.easeNone,
              repeat: -1
            ...
            new TimelineMax({ repeat: -1 })
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#f8d4e4", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#d8ebe7", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#5ebfaf", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#577081", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#eb4333", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#d7ebe6", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#bfbdce", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#9ccad5", ease: Power0.easeNone }
              )
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
              ...
              .to(
                ".homepage__description_bg .h, .homepage__description_bg .m",
                0.5,
                { fill: "#2f2f2f", ease: Power0.easeNone }
              );
          }
        })
        .staggerFrom(
          document.querySelectorAll(".homepage__hero h1 span"),
          1,
          { rotationX: 90, opacity: 0, ease: Power2.easeOut },
          0.1,
          "+=0.3",
          swichText
        )
        ...
        .add(function() {
          ...
          TweenMax.set(
            document.querySelectorAll(
              ".homepage__insta h2 span, .expert__form_title h2 span"
            ),
            { opacity: 0 }
          );
        });
      function swichText() {
        ...
        new TimelineMax()
          .to(
            [app.homepage.hero.container, app.homepage.hero.container2],
            0.2,
            { alpha: 1 }
          )
          .to(document.querySelectorAll(".homepage__hero h1 span"), 0.2, {
            opacity: 0
          })
  ...
  function heroRender() {
    ...
    loader.load(function(loader, resources) {
      ...
      that.choise.x =
        (window.innerWidth / 100) * 42 + (window.innerWidth / 100) * 6;
      that.choise.y = 140 - (fs / 100) * 7.563724377883669 + Math.floor(fs);
      that.save.x =
        (window.innerWidth / 100) * 42 + (window.innerWidth / 100) * 12;
      that.save.y =
        140 - (Math.floor(fs) / 100) * 7.563724377883669 + Math.floor(fs) * 2;
      that.time.x =
        (window.innerWidth / 100) * 42 + (window.innerWidth / 100) * 18;
      that.time.y =
        140 - (Math.floor(fs) / 100) * 7.563724377883669 + Math.floor(fs) * 3;
      ...
      const texture = app.homepage.hero.hero.renderer.generateTexture(
        circle,
        PIXI.SCALE_MODES.NEAREST,
        1,
        bounds
      );
      const texture2 = app.homepage.hero.hero.renderer.generateTexture(
        rect,
        PIXI.SCALE_MODES.NEAREST,
        1,
        bounds2
  ...
  function dragRender() {
    ...
    this.el.querySelectorAll("h2 a").forEach(function(el, i) {
      el.addEventListener("click", function(e) {
        ...
        TweenMax.to('[data-action="homepage-drag"]', 0.6, {
          scale: 0,
          ease: Power2.easeInOut
        });
        if (target == "leisure") {
    ...
    this.pat1.onload = function() {
      ...
      that.ctx1.lineTo(
        that.el.clientWidth / 2 + that.el.clientWidth / 2 / 2.92,
        0
      );
      that.ctx1.lineTo(
        that.el.clientWidth / 2 - that.el.clientWidth / 2 / 2.92,
    ...
    this.dragControl = function(e) {
      if (that.dragMove) {
        ...
        if (x < 0 && x > -limit) {
          ...
          let href;
          TweenMax.to(
            '[data-action="homepage-drag"], .homepage__drag_dots',
            0.6,
            { scale: 0, ease: Power2.easeInOut }
          );
          if (x < -limit) {
            href = document.querySelector(
              '.homepage__drag h2 a[data-target="leisure"]'
            ).href;
            TweenMax.to(document.querySelector("canvas.leisure"), 1.2, {
              x: -that.el.clientWidth,
              ease: Power2.easeInOut,
            ...
            href = document.querySelector(
              '.homepage__drag h2 a[data-target="business"]'
            ).href;
            TweenMax.set(document.querySelector("canvas.leisure"), { x: 0 });
            TweenMax.to(document.querySelector("canvas.business"), 1.2, {
              x: that.el.clientWidth,
  ...
  this.resizeHomepage = function(e) {
    ...
    //that.hero.heroBgCover();
    that.hero.hero.renderer.resize(
      that.hero.el.clientWidth,
    ...
    that.hero.choise.x =
      (that.hero.el.clientWidth / 100) * 42 +
      (that.hero.el.clientWidth / 100) * 6;
    that.hero.choise.y = 140 - (fs / 100) * 7.563724377883669 + Math.floor(fs);
    that.hero.save.x =
      (that.hero.el.clientWidth / 100) * 42 +
      (that.hero.el.clientWidth / 100) * 12.1;
    that.hero.save.y =
      140 - (Math.floor(fs) / 100) * 7.563724377883669 + Math.floor(fs) * 2;
    that.hero.time.x =
      (that.hero.el.clientWidth / 100) * 42 +
      (that.hero.el.clientWidth / 100) * 18.2;
    that.hero.time.y =
      140 - (Math.floor(fs) / 100) * 7.563724377883669 + Math.floor(fs) * 3;
    ...
    that.drag.ctx1.lineTo(
      that.drag.el.clientWidth / 2 + that.drag.el.clientWidth / 2 / 2.92,
      0
    );
    that.drag.ctx1.lineTo(
      that.drag.el.clientWidth / 2 - that.drag.el.clientWidth / 2 / 2.92,
    ...
    // Clip to the current path
    that.drag.ctx1.clip();
    that.drag.ctx1.drawImage(
      that.drag.pat1,
      0,
      0,
      that.drag.el.clientWidth,
      that.drag.el.clientHeight
    );
  ...
  this.eventsInit = function() {
    ...
    document
      .querySelector('[data-action="homepage-drag"]')
      .addEventListener("mousedown", function(e) {
        if (app.homepage.drag.show) {
          ...
          TweenMax.to(document.querySelector(".homepage__drag_dots"), 0.8, {
            scaleX: 1,
            ease: Power4.easeOut
          });
    ...
    document
      ...
      .addEventListener("mouseup", function(e) {
        if (!that.drag.dragDone) {
          ...
          that.drag.dragx = 0;
          TweenMax.to('[data-action="homepage-drag"]', 0.5, {
            x: 0,
            ease: Power2.easeOut
          });
          TweenMax.to(
            [
              document.querySelector("canvas.leisure"),
              document.querySelector("canvas.business")
            ],
            0.5,
    ...
    document
      .querySelector(".homepage__drag")
      .addEventListener("mousemove", that.drag.dragControl);
    document
      .querySelector('[data-action="homepage-hero-play"]')
      .addEventListener("mouseenter", function(e) {
        ...
        TweenMax.to(this.querySelector(".icon"), 0.5, {
          fill: "#fff",
          ease: Power3.easeOut
        });
        TweenMax.to(".cursor span", 0.5, { scale: 0, ease: Power3.easeOut });
    ...
    document
      ...
      .addEventListener("mouseleave", function(e) {
        ...
        if (app.homepage.heroVideo.active) {
          ...
          TweenMax.to(".cursor i.circle", 0.5, {
            scale: 0,
            ease: Power3.easeIn
          });
          if (app.homepage.heroVideo.video.paused) {
            TweenMax.to(".cursor i.play", 0.5, {
              scale: 1,
              ease: Power3.easeIn
            });
          } else {
            TweenMax.to(".cursor i.pause", 0.5, {
              scale: 1,
              ease: Power3.easeIn
            });
          }
    ...
    document
      ...
      .addEventListener("mouseleave", function(e) {
        ...
        TweenMax.to(this.querySelector("i"), 0.6, {
          scale: 0,
          ease: Power4.easeIn
        });
        TweenMax.to(document.querySelector(".homepage__drag_dots"), 0.4, {
          scaleX: 0,
          ease: Power3.easeIn
        });
    ...
    document
      .querySelector('[data-action="learn-more"]')
      .addEventListener("mouseenter", function(e) {
        TweenMax.to(this.querySelector("i"), 0.5, {
          scale: 1,
          ease: Power3.easeOut
    ...
    document
      .querySelectorAll(".slider-button-prev, .slider-button-next")
      .forEach(function(el, i) {
        el.addEventListener("mouseenter", function(e) {
  ...
  function buttonsRender() {
    document.querySelectorAll(".button-circle").forEach(function(el, i) {
      ...
      var deg = 120 / splitTop.chars.length,
        origin = 0,
        radius = 75;
      ...
      if (el.getAttribute("data-hover")) {
        ...
        textBlock.appendChild(bot);
        el.appendChild(textBlock);
        var deg = 100 / splitTop.chars.length,
          origin = 0,
          radius = 75;
        for (var i in splitTop.chars) {
          splitTop.chars[i].style.height = radius + "px";
  ...
  this.cursor = {
    init: function() {
      TweenMax.set(".cursor", {
        x: document.body.clientWidth / 2 - 35,
      ...
      document
        .querySelector(".homepage__feedback")
        .addEventListener("mouseenter", function(e) {
          TweenMax.to(".cursor", 0.5, { borderColor: "#ee412a" });
          TweenMax.to(".cursor i.circle", 0.5, { backgroundColor: "#ee412a" });
        });
  ...
  this.tabs = {
    ...
    init: function() {
      ...
      this.render();
      document
        .querySelectorAll(".homepage__tabs_list span")
        .forEach(function(el, i) {
          el.addEventListener("click", function(e) {
      ...
      document
        .querySelectorAll(".homepage__tabs_content h3 i")
        .forEach(function(el, i) {
          el.addEventListener("click", function(e) {
            that.onClose(e);
          });
    ...
    onHover: function(e) {
      if (!this.active) {
        if (e.type == "mouseenter") {
          ...
          TweenMax.to(
            document.querySelectorAll(".homepage__tabs_list span:not(.active)"),
            1,
            { opacity: 0.1, color: "#2f2f2f", ease: Power2.easeOut }
          );
        } else {
          TweenMax.to(
            document.querySelectorAll(".homepage__tabs_list span"),
            0.8,
            { opacity: 1, color: "#ee412a", ease: Power2.easeOut }
          );
    ...
    onActive: function(e) {
      ...
      let that = this;
      let target = e.target.getAttribute("data-target");
      TweenMax.to(".homepage__tabs_disk .disk--arrow-1", 1.5, {
        rotation: 360,
        ease: Power3.easeInOut
      });
      TweenMax.to(".homepage__tabs_disk .disk--arrow-2", 1.5, {
        rotation: 360,
        ease: Power3.easeInOut
      });
      TweenMax.to(".homepage__tabs_disk .disk--text", 1.5, {
        ...
        onComplete: function() {
          ...
          TweenMax.set(
            ".homepage__tabs_disk .disk--arrow-1, .homepage__tabs_disk .disk--arrow-2",
            { clearProps: "all" }
          );
        }
      });
      ...
      tl.staggerTo(
        ...
        .to(
          document.querySelector(".homepage__tabs_list span.active"),
          0.7,
          { opacity: 0, ease: Power2.easeOut },
          "-=0.8"
        )
        .set(
          '.homepage__tabs_content [data-tab="' + target + '"]',
          { display: "block", zIndex: 2, position: "relative" },
          "-=0.5"
        )
        .fromTo(
          '.homepage__tabs_content [data-tab="' + target + '"] h3 div',
          0.8,
          { rotationX: 90, opacity: 0 },
          { rotationX: 0, opacity: 1, ease: Power2.easeOut }
        )
        .fromTo(
          '.homepage__tabs_content [data-tab="' + target + '"] p',
          1.5,
          { opacity: 0 },
    ...
    onClose: function(e) {
      ...
      TweenMax.to(".homepage__tabs_disk .disk--text", 1.5, {
        color: "#9BCAD6",
        onComplete: function() {
          TweenMax.to(".homepage__tabs_disk .disk--arrow-1", 90, {
            rotation: 360,
            repeat: -1,
            ease: Power0.easeNone
          });
          TweenMax.to(".homepage__tabs_disk .disk--arrow-2", 15, {
            rotation: 360,
            repeat: -1,
            ease: Power0.easeNone
          });
          ...
          document
            .querySelector(".homepage__tabs_list .active")
      ...
      tl.to(
        ...
        .to(
          e.target.closest(".tab--body").querySelector("p"),
          1,
          { opacity: 0, ease: Power2.easeOut },
          "start"
        )
        .set(e.target.closest(".tab--body"), { clearProps: "all" })
        .set(document.querySelectorAll(".homepage__tabs_list span"), {
          color: "#ee412a"
        })
    ...
    render: function() {
      ...
      var split = new SplitText(textBlock, {
        type: "chars",
        reduceWhiteSpace: false
      });
      el.innerHTML = "";
      el.appendChild(textBlock);
      var deg = 360 / split.chars.length,
        origin = 0,
        radius = 320;
      ...
      this.diskTextAnim = TweenMax.to(".homepage__tabs_disk .disk--text", 60, {
        rotation: 360,
        repeat: -1,
        ease: Power0.easeNone
      });
      ...
      document
        .querySelectorAll(".homepage__tabs_content .tab--body h3 div")
        .forEach(function(el, i) {
      ...
      TweenMax.set(document.querySelectorAll(".homepage__tabs_list span"), {
        opacity: 0
      });
    }
  };
  ...
  function aosInit() {
    ...
    document.addEventListener("aos:in", function(e) {
      if (e.detail.classList.contains("homepage__drag")) {
        new TimelineMax()
          .to(
            [".homepage__drag .blind-left", ".homepage__drag .blind-right"],
            0.8,
            { scaleX: 0, ease: Power2.easeIn }
          )
          .staggerFrom(
            document.querySelectorAll(
              ".homepage__drag canvas.leisure, .homepage__drag canvas.business"
            ),
            1.6,
            { scale: 1.4 },
            0.1,
          ...
          .staggerFrom(
            document.querySelectorAll(
              ".homepage__drag h2 a, .homepage__drag h2 span"
            ),
            1.5,
            { rotationX: 90, opacity: 0, ease: Power3.easeOut },
            0.25,
            "-=0.8"
          )
          .to(
            ".homepage__drag h2.or span",
            0.8,
            {
              scale: 0,
              opacity: 0,
          ...
          .from(
            ".homepage__drag .button-circle",
            0.8,
            { scale: 1.4, opacity: 0, ease: Power3.easeOut },
            "-=0.6"
          )
          .add(function() {
      ...
      if (
        e.detail.classList.contains("homepage__feedback_slider") &&
        e.detail.classList.contains("leisure")
      ) {
        new TimelineMax()
          .set(e.detail, { visibility: "visible" })
          .fromTo(
            e.detail.querySelector(".swiper-slide-active .slide--photo"),
            1.4,
            { x: "100%", scale: 1.3, opacity: 0 },
            { x: "0%", scale: 1, opacity: 1, ease: Power4.easeOut },
            "start"
          )
          .fromTo(
            e.detail.querySelector(".swiper-slide-active .slide--photo"),
            1.4,
            { skewX: -30 },
            { skewX: 0, ease: Power4.easeOut },
            "start"
          )
          .fromTo(
            ".homepage__feedback_slider.leisure h2 span",
            1,
            { opacity: 0, rotationX: 90 },
            { opacity: 1, rotationX: 0, ease: Power2.easeOut },
            "-=0.7"
          )
          .fromTo(
            e.detail.querySelector(".swiper-slide-active .slide--text"),
            1,
            { opacity: 0, y: "50%" },
            { opacity: 1, y: "0%", ease: Power2.easeOut },
            "-=0.6"
          ...
          .fromTo(
            ".homepage__feedback_slider.leisure .slider-control",
            1,
            { opacity: 0, scale: 1.3 },
      ...
      if (
        e.detail.classList.contains("homepage__feedback_slider") &&
        e.detail.classList.contains("business")
      ) {
        new TimelineMax()
          .set(e.detail, { visibility: "visible" })
          .fromTo(
          ...
          .fromTo(
            ".homepage__feedback_slider.business h2 span",
            1,
            { opacity: 0, rotationX: 90 },
            { opacity: 1, rotationX: 0, ease: Power2.easeOut },
            "-=0.7"
          ...
          .fromTo(
            ".homepage__feedback_slider.business .slider-control",
            1,
            { opacity: 0, scale: 1.3 },
            { opacity: 1, scale: 1, ease: Power2.easeOut },
            "-=0.6"
          ...
          .fromTo(
            ".homepage__feedback .button-circle",
            1,
            { opacity: 0, scale: 1.4 },
            { opacity: 1, scale: 1, ease: Power2.easeOut },
            "-=1"
      ...
      if (e.detail.classList.contains("homepage__services")) {
        new TimelineMax()
          .to(
            e.detail.querySelectorAll(".swiper-slide-active .slide--photo"),
            1.6,
            { scale: 1 }
          )
          .staggerFrom(
            e.detail.querySelectorAll("h2"),
            1,
            { opacity: 0, scale: 1.3, ease: Power2.easeOut },
            0.3,
            "-=1"
          )
          .staggerFrom(
            e.detail.querySelectorAll(".swiper-slide-active h3 a"),
            1.5,
            { rotationX: 90, opacity: 0, ease: Power3.easeOut },
            0.3,
            "-=0.8"
          ...
          .staggerFrom(
            e.detail.querySelectorAll(".slider-control"),
            1,
            { opacity: 0, scale: 1.4, ease: Power2.easeOut },
            0.3,
            "-=1"
          );
      }
      ...
      if (
        e.detail.classList.contains("leisure") &&
        e.detail.closest(".homepage__insta")
      ) {
        TweenMax.fromTo(
          e.detail.querySelectorAll("h2 span"),
          1.5,
          { rotationX: 90, opacity: 0 },
          { rotationX: 0, opacity: 1, ease: Power3.easeOut }
        );
      }
      if (
        e.detail.classList.contains("business") &&
...
const _catalog = function() {
  ...
  this.eventsInit = function() {
    ...
    document
      .querySelectorAll(".catalog__filter_nav li")
      .forEach(function(el, i) {
        el.addEventListener("click", function(e) {
          ...
          if (
            document.querySelectorAll(".catalog__filter_list .active").length
          ) {
            let current =
              document.querySelector(".catalog__filter_list .active").id ==
              e.currentTarget.getAttribute("data-target");
            TweenMax.to(
              document.querySelector(".catalog__filter_list .active"),
              0.3,
              {
                height: 0,
                opacity: 0,
                onComplete: function() {
                  ...
                  document
                    .querySelector(".catalog__filter_list .active")
                    .classList.remove("active");
                  if (!current) {
...
const _leisure = function() {
  ...
  this.hero = {
    ...
    render: function() {
      ...
      const loader = PIXI.Loader.shared;
      document
        .querySelectorAll(".leisure__hero_backgrounds img")
        .forEach(function(el, i) {
      ...
      loader.load(function(loader, resources) {
        ...
        that.slide1.alpha = 1;
        TweenMax.set('.leisure__hero h1 .switch[data-slide="1"]', {
          display: "block"
        });
        document.querySelector(
          ".leisure__hero_pagination .pagination--slide .current"
        ).innerHTML = that.slide < 10 ? "0" + that.slide : that.slide;
        document.querySelector(
          ".leisure__hero_pagination .pagination--slide .all"
        ).innerHTML = that.slides < 10 ? "0" + that.slides : that.slides;
        ...
        function onPointerMove(e) {
          TweenMax.to(that.heroFilter.center, 1, {
            x: e.clientX,
            y: e.clientY
          });
        }
        ...
        function onPointerClick(e) {
          ...
          new TimelineMax()
            .to(that.heroFilter, 1.3, { innerRadius: r, ease: Power4.easeIn })
            .set(that.heroFilter, { innerRadius: 0, strength: 0 })
            .to(
              ".leisure__hero_pagination .pagination--bar i",
              1,
              { width: pS + "%", ease: Power3.easeIn },
              "transition"
            )
            .to(
              that.heroFilter,
              1,
              { strength: 2, ease: Power3.easeIn },
              "transition"
            )
            .to(current, 1, { alpha: 0, ease: Power3.easeIn }, "transition")
            .to(next, 1, { alpha: 1, ease: Power3.easeIn }, "transition")
            .fromTo(
              '.leisure__hero h1 .switch[data-slide="' + that.slide + '"]',
              1,
              { rotationX: 0, opacity: 1, y: 0 },
              { rotationX: 90, opacity: 0, y: 30, ease: Power3.easeIn },
              "transition"
            )
            .set('.leisure__hero h1 .switch[data-slide="' + that.slide + '"]', {
              display: "none"
            })
            .set(
              '.leisure__hero h1 .switch[data-slide="' +
                (that.slide < that.slides ? that.slide + 1 : 1) +
                '"]',
              { display: "block" }
            )
            .add(function() {
              ...
              if (that.slide < that.slides) {
                slide =
                  that.slide + 1 < 10 ? "0" + (that.slide + 1) : that.slide + 1;
              } else {
                slide = "01";
              }
              document.querySelector(
            ...
            .fromTo(
              '.leisure__hero h1 .switch[data-slide="' +
                ...
                '"]',
              1,
              { rotationX: -90, opacity: 0, y: 30 },
              { rotationX: 0, opacity: 1, y: 0, ease: Power3.easeOut },
              "end"
            )
            .to(
              that.heroFilter,
              1,
              { strength: 0.1, ease: Power3.easeOut },
              "end"
            )
            .to(
              that.heroFilter,
              1,
              { innerRadius: 200, ease: Power3.easeOut },
              "end"
            )
            .add(function() {
              if (that.slide < that.slides) {
  ...
  this.tabs = {
    ...
    slides: document.querySelectorAll(
      ".leisure .homepage__tabs .disk--icons img[data-slide]"
    ).length,
    slide: 1,
    transition: false,
    init: function() {
    ...
    render: function() {
      ...
      document
        .querySelector(
          '.leisure .homepage__tabs .disk--icons img[data-slide="1"]'
        )
        .classList.add("current");
      this.animation = new TimelineMax({ repeat: -1 });
      this.animation
        .set(".leisure .homepage__tabs .disk--icons .shape", {
        ...
        .to(".leisure .homepage__tabs .disk--icons .shape", 0.8, {
          scaleX: 1,
          ease: Power4.easeInOut
        })
        .add(function() {
          document
            .querySelector(".leisure .homepage__tabs .disk--icons img.current")
  ...
  this.loader = {
    ...
    loaded: function() {
      ...
      this.singleSlider.on("init", function() {
        ...
        function heroBgCover(el) {
          ...
          if ((that.el.clientWidth + 325 * 2) / that.el.clientHeight > ratio) {
            return {
              ...
              x: 0,
              y:
                (window.innerHeight - (that.el.clientWidth + 325 * 2) / ratio) /
                2
            };
          } else {
            return {
              ...
              height: that.el.clientHeight,
              x:
                (that.el.clientWidth + 325 * 2 - that.el.clientHeight * ratio) /
                2,
              y: 0
            };
          }
        ...
        this.el
          .querySelectorAll(".swiper-slide .slide--photo img")
          .forEach(function(el, i) {
            let src = el.getAttribute("data-src");
            ...
            if (
              i ==
              that.el.querySelectorAll(".swiper-slide .slide--photo img")
                .length -
                1
            ) {
      ...
      this.singleSlider.on("slideChangeTransitionStart", function() {
        ...
        if (this.realIndex > this.previousIndex) {
          this.images.forEach(function(el, i) {
            TweenMax.to(el.skew, 0.5, {
              x: -0.6,
              ease: Power2.easeIn,
              yoyo: true,
              repeat: 1
            });
            TweenMax.to(el.scale, 0.5, {
              x: 1.5,
              y: 1.5,
              ease: Power2.easeIn,
              yoyo: true,
          ...
          new TimelineMax()
            .to(
              this.rootContainer,
              1,
              { x: -x, ease: Power2.easeInOut },
              "start"
            )
            .to(
              this.el.querySelectorAll(".swiper-slide .slide--content"),
              0.5,
              {
                skewX: -34,
                scale: 1.2,
